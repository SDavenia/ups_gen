...
def plot_shaded_bars(df, column_a, column_b, short_column_a_chars=None, decision_map=None, remove_none=False):
    """
    ...
    remove_none (bool, optional): If True, removes rows where column_b is 'None' for plotting the percentages. Defaults to False
    """
    ...
    if decision_map:
        ...
        weighted_avgs = counts.groupby(column_a).apply(
            lambda x: (x['count'] * x['mapped_value']).sum() / x['count'].sum(), include_groups=False
        )
    ...
    for col in pivot_percent.columns:
        ax.bar(x_ticks, pivot_percent[col], bottom=bottom, label=str(col))
