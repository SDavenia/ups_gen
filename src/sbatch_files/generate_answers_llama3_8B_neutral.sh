...
#SBATCH -c 8
#SBATCH --gres=gpu:1
#SBATCH --output=sbatch_files/output/generate_answers_llama3_8B_neutral.out
#SBATCH --job-name generate_answers_llama3_8B_neutral
...
for add_context_key in "wiki_mus_classical" "wiki_mus_heavy-metal" "wiki_mus_jazz" "wiki_mus_hip-pop" "wiki_mus_rock" "wiki_mus_pop" "wiki_obj_table" "wiki_obj_chair" "wiki_obj_sink" "wiki_pol_trump" "wiki_pol_obama" "wiki_pol_biden" "wiki_pol_bush"; do
    for add_context_pos in "system-beginning" "system-end" "user-beginning" "user-end"; do
        ...
        python generate_answers.py --model_id $model_id --batch_size $batch_size --seed $seed --additional_context_key $add_context_key --additional_context_placement $add_context_pos --prompt_question_type "open_domain_neutral"
    done
done